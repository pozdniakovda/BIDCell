...
def get_paths(config, make_new, learning_rate, dynamic_solvers, selected_solver=None, 
    ...
    if not dynamic_solvers: 
        print(f"selected_solver = {selected_solver}")
        experiment_path = os.path.join(config.files.data_dir, "model_outputs", f"{timestamp}_{selected_solver}_lr-{learning_rate}")
    else:
        print(f"starting_solver = {starting_solver}")
        print(f"ending_solver = {ending_solver}")
        print(f"epochs_before_switch = {epochs_before_switch}")
    ...
    if training_repeats > 1: 
        ...
        for i in np.arange(1, training_repeats+1): 
            test_output_dir = os.path.join(experiment_path, f"repeat_{i}", config.experiment_dirs.test_output_dir)
            ...
            repeat_paths[i] = {"repeat_path": os.path.join(experiment_path, f"repeat_{i}"), 
                               ...
                               "test_output_path": test_output_dir}

    else:
        test_output_dir = os.path.join(experiment_path, config.experiment_dirs.test_output_dir)
        make_dir(test_output_dir)
...
def predict(config: Config) -> str:
    ...
    for training_repeat in np.arange(1, training_repeats+1): 
        ...
        model_dir = paths_dict[training_repeat]["model_path"] if training_repeats > 1 else paths_dict["model_path"]
        test_output_dir = paths_dict[training_repeat]["test_output_path"] if training_repeats > 1 else paths_dict["test_output_path"]
        test_output_dirs[training_repeat] = test_output_dir
    
        # Set up the model
        logging.info(f"Initialising model for repeat #{training_repeat}")
        ...
        logging.info(f"Finished prediction for repeat #{training_repeat}")

    return test_output_dirs
